...
class PostProcess(nn.Module):
    ...
    def forward(self, outputs, target_sizes):
        ...
        out_adjs,out_logits_grasp=outputs['pred_adj'],outputs['pred_logits_grasp']
...
def calculate_adj(all_gt_adj, all_dt_adj):
    ...
    for k in range(len(all_dt_adj)):
        ...
        for i in range(len(all_gt_adj[k][0])):
            for j in range(len(all_gt_adj[k][0])):
                gt_adj.update({(all_gt_adj[k][0][i],all_gt_adj[k][0][j]):all_gt_adj[k][1][i][j]})
        ...
        for key_dt in dt_adj.keys():
            ...
            for key_gt in gt_adj.keys():
                if key_dt==key_gt and dt_adj[key_dt]>0.5 and gt_adj[key_gt]==1:
                    TP+=1
        for key_gt in gt_adj.keys():
                if gt_adj[key_gt]==1:
                    TR_gt+=1
    ...
    Image_Accuracyi=[]
    for i in range(4):
        Image_Accuracyi.append(IAi[i]/Ii[i])
    return precision,recall,Image_Accuracy,Image_Accuracyi
def main():
    main_args = get_main_args_parser().parse_args()
    
    # 加载模型
    model = load_model('/home/pmh/code/DAB-DETR-GRASP/results/checkpoint0134_beforedrop.pth', main_args)
    model.eval()
    # coco_evaluator = CocoEvaluator(base_ds, iou_types)
    # 构建数据集和数据加载器
    dataset = build_dataset(image_set='val')
    data_loader = DataLoader(dataset, batch_size=4, collate_fn=utils.collate_fn)
        ...
        for i in range(len(all_gt)):
            ...
            if all_gt[i]['image_id'] == all_dt[j]['image_id'] and all_gt[i]['category_id'] == all_dt[j]['category_id']:
                ...
                if iou > 0.5:
                    ...
                    continue
    FP = len(all_dt) - len(used_all_gt_indices) 
    FN = len(all_gt) - len(used_all_gt_indices) 
    # 打印整个数据加载器的总结果
        ...
        for i in range(len(all_gt_grasp)):
            ...
            if all_gt_grasp[i]['image_id'] == all_dt_grasp[j]['image_id'] and all_gt_grasp[i]['category_id_grasp'] == all_dt_grasp[j]['category_id_grasp']:
                ...
                if iou > 0.25 and angle_diff<30:
                    ...
                    continue
    FP1 = len(all_dt_grasp) - len(used_all_gt_grasp_indices) 
    FN1 = len(all_gt_grasp) - len(used_all_gt_grasp_indices) 
    # 打印整个数据加载器的总结果
