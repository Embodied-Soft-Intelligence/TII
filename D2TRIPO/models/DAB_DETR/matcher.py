...
from util.box_ops_grasp import generalized_grasp_box_cost
...
class HungarianMatcher_Grasp(nn.Module):
    ...
    def forward(self, outputs, targets):
        ...
        out_angle_grasp = outputs["pred_angles_grasp"].flatten(0, 1)  # [batch_size * num_queries, 1]
        ...
        cost_grasp = generalized_grasp_box_cost(out_bbox_grasp, tgt_xywh, out_angle_grasp, tgt_angle)
        ...
        # Final cost matrix
        C = self.cost_grasp_box * cost_bbox + self.cost_grasp_class * cost_class + self.cost_grasp * cost_grasp
        C = C.view(bs, num_queries, -1).cpu()

        sizes = [len(v["grasp_angles"]) for v in targets]
        indices = [linear_sum_assignment(c[i]) for i, c in enumerate(C.split(sizes, -1))]
        return [(torch.as_tensor(i, dtype=torch.int64), torch.as_tensor(j, dtype=torch.int64)) for i, j in indices]

def build_matcher(args):
    return  HungarianMatcher_Grasp(
            cost_grasp_class = args.set_cost_grasp_class,
            cost_grasp = args.set_cost_grasp, 
